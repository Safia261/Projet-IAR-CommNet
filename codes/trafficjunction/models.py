...
class DiscreteCommPolicy(nn.Module):
    """Discrete communication baseline
    ...
    def forward(self, obs: torch.Tensor, mask: torch.Tensor, comm_mode: str = "sample", state=None) -> Dict[str, torch.Tensor]:
        B, N, _ = obs.shape
        ...
        return {
            ...
            "comm_vec": c,  # last OR-aggregated channel (for debugging)
            "next_state": None,
        }
...
class CommNetContinuous(nn.Module):
    ...
    def forward(self, obs: torch.Tensor, mask: torch.Tensor, state=None) -> Dict[str, torch.Tensor]:
        ...
        return {
            ...
            "hidden": h, # final hidden per agent
            "next_state": None,
        }
